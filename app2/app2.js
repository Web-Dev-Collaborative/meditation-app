...
playAudioBtn.addEventListener( "click", () => {
  if ( playAudioBtn.classList.contains( "muted" ) ) {
    ...
    const children = document.querySelectorAll( "#switch-audio .fa-volume-mute" );
    document.querySelectorAll( "#switch-audio .fa-volume-up" )[ 0 ].style.display =
      "none";
    document.querySelectorAll(
      "#switch-audio .fa-volume-mute"
...
document.getElementById( "config-md" ).addEventListener( "click", ({target}) => {
...
for ( let i = 0; i < swatches.length; i++ ) {
  swatches[ i ].addEventListener( "click", ({target}) => {
    Array.prototype.forEach.call( swatches, ({classList}) => {
      classList.remove( "active" );
    } );
    let theme = target.getAttribute( "data-theme" );
    const f = themes[ parseInt( theme ) ].first;
    const s = themes[ parseInt( theme ) ].sec;
...
const openFile = ({target}) => {
  ...
  reader.onload = () => {
    ...
    const output = document.getElementById( "OpenImgUpload" );
    const mainOutput = document.getElementById( "mainBg" );
    output.src = dataURL;
...
setInterval( () => {
  Array.prototype.forEach.call( poses, ({classList}) => {
...
document.getElementById( "toggly" ).addEventListener( "click", () => {
